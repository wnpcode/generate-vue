      ...
      <div v-if="showParentPage">
        <div class="breadcrumbs">
          <nav style="display: inline">
            ...
            <li><router-link class="breadcrumbs-active" :to="{name: 'company', params:{} }"> company </router-link></li>
          </nav>
        </div>

        <div class="row">
          <div class="col">
            <router-link class="btn btn-primary create-button" :to="{ name: 'companyCreate', params: {} }">Create <i class="fa fa-plus" aria-hidden="true"></i></router-link>
          </div>
        </div>
...
<th @click="sortBy('address')"> address <i style="float: right" class="fa fa-sort"> </i></th>
...
<td>{{company.active}}</td>
    ...
    <td>
      <div class="options-button">
        <button class="btn btn-info" @click="view(company.id)" >
          <i class="fa fa-eye" aria-hidden="true"></i>
        </button>
        <button class="btn btn-warning" @click="edit(company.id)" >
          <i class="fa fa-edit" aria-hidden="true"></i>
        </button>
        <button class="btn btn-danger" @click="remove(company.id)">
    ...
    export default {
      ...
      computed: {
        ...mapGetters("company", ["companyProcessed", "getCompanies"]),
      },
      methods: {
        ...mapActions("company", ["deleteCompany", "fetchCompanies"]),
        view(id){
          this.$router.push({ name: 'companyView', params: { id: id }})
        },
        edit(id){
          this.$router.push({ name: "companyEdit", params: { id: id }})
        },
        async remove(id){
          let option = await this.$modal.show({title: "Danger", message: "Do you want delete this company?" , alert : "danger"});
          if(option){
            this.deleteCompany(id)
              .then( response => {
                this.$modal.show({title: "Success", message: "company was deleted with successfull!", alert: "info"});
                this.getResources();
              }).catch(err => {
        ...
        getResources () {
          if (this.companyProcessed || this.mainList.length === 0) {
            this.fetchCompanies()
              .then(response => {
            ...
            this.mainList = this.getCompanies;
          }
        },
        sortBy(param){
      ...
      watch: {
        'pagination.current': function(value){
          ...
          this.companyList = this.mainList.slice((this.pagination.current * this.pagination.numberRegisterForPage), ((this.pagination.current * this.pagination.numberRegisterForPage) + this.pagination.numberRegisterForPage ));
        },
        'pagination.numberRegisterForPage': function(){
            this.pagination.current = -1;
        },
        'mainList': function(value){
          this.companyList = value.slice((this.pagination.current * this.pagination.numberRegisterForPage), ((this.pagination.current * this.pagination.numberRegisterForPage) + this.pagination.numberRegisterForPage ));
        },
        'search': function( text){
          this.companyList = this.mainList.filter( object => JSON.stringify(object).includes(text))
        }
      },
      created() {
        this.getResources();

        this.companyList = this.mainList.slice(0,10);
        this.pagination.numberPages = parseInt(this.mainList.length / this.pagination.numberRegisterForPage);
      },
